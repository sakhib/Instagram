...
import ru.clearline.instagram.common.task
...
import ru.clearline.instagram.data.firebase.common.*
            ...
            task { taskSource ->
                storage.child("users").child(uid).child("images")
                        .child(imageUri.lastPathSegment).putFile(imageUri).addOnCompleteListener {
                            if (it.isSuccessful) {
                                storage.child("users/$uid/images").downloadUrl.addOnCompleteListener {
    ...
    override fun uploadUserPhoto(localImage: Uri): Task<Uri> =
            task { taskSource ->
                storage.child("users/${currentUid()!!}/photo").putFile(localImage)
                        .addOnSuccessListener {
                            storage.child("users/${currentUid()!!}/photo").downloadUrl.addOnCompleteListener {
                                taskSource.setResult(it.result)
                            }
                        }
            }
